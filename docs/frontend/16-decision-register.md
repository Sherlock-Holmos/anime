...
| FED-008 | Bangumi 评分只读展示 | 当前不设计自有评分机制 | 不做评分输入、排名、写 API |
...
| ID | 能力 | 首发处理 |
|---|---|---|
| FED-D01 | Anime 自有评分和排行 | 不展示入口、不预留空卡片 |
| FED-D02 | 视频播放/下载 | 剧集只展示元数据 |
| FED-D03 | 私信、关注、动态流 | 不创建空接口 |
...
`FES-1.3` 发布时没有阻塞开发的开放项。开发中新增开放项格式为 `OPEN-YYYY-NNN`，必须包含：问题、可选方案、默认临时行为、影响模块、决策截止点和 Owner。只有默认行为不会改变公共合同或用户数据时，才允许带着开放项继续实现。
