...
| iOS 26+ App Shell 与根导航 | `app/iosApp/iosApp/ContentView.swift`、`shared/app/src/iosMain/.../IosBridge.kt`、`shared/app/.../AnimeApp.kt`、`app/iosApp/iosApp.xcodeproj/project.pbxproj` | `animeCheck`、GitHub Actions iOS Simulator/device build、iOS 26 真机点击回归 | 根底栏使用 SwiftUI 官方 Liquid Glass；选中索引通过 `IosBridge` 双向同步，Compose iOS 层不创建 Kyant Backdrop。Android/Desktop/Web 继续使用 Kyant 组件；iOS 真机截图与手势回归仍需 GitHub Actions/macOS 验证 |
| Navigation 3 页面栈 | `core/navigation/.../AppRoute.kt`、`AppNavigator.kt`、`shared/app/.../AnimeApp.kt` | `AppNavigatorTest`、Android 编译 | 四根栈、轻量可序列化路由、保存恢复、NavEntry ViewModel 生命周期及发现→详情→返回已落地；Deep Link、AuthGate、进程恢复和设备级 UI-NAV 仍属于 F3 |
| 发现页 Repository 纵向切片 | `data/catalog/.../FixtureCatalogRepository.kt`、`feature/discover/.../DiscoverRoute.kt`、`DiscoverViewModel.kt`、`DiscoverReducer.kt`、`DiscoverUiMapper.kt`、`DiscoverScreen.kt`、`shared/app/.../AppContainer.kt` | `FixtureCatalogRepositoryContractTest`、`DiscoverReducerTest`、`animeCheck`、`assembleDemoDebug` | Demo 已移除页面内静态 Fixture，改由正式 `CatalogRepository` 状态流驱动；首次骨架、保留内容刷新、空/离线/阻断错误 UI、4 分区、继续观看横卡及详情路由已落地。Fixture 当前为领域记录映射，JSON DTO 直读/SQLDelight、场景面板、Snackbar、分区路由和 Compose UI/截图测试仍属于 F2/F4 |
| 条目详情首个纵向切片 | `feature/subject/.../SubjectRoute.kt`、`SubjectViewModel.kt`、`SubjectScreen.kt`、`data/catalog/.../FixtureCatalogRepository.kt` | `CT-CAT-003`、Android 编译 | 发现作品可进入沉浸式详情，读取确定性详情 Fixture 并显示 Bangumi 只读评分、简介和作品信息；其余 Section、收藏/进度、离线及完整 UI 测试仍属于 F6 |
