...
> 维护说明：本文是按提交记录整理的历史增量，不等同于当前 HEAD 的发布证明。当前实现状态、质量门禁和发布判断以 [`../product/01-product-management-baseline.md`](../product/01-product-management-baseline.md) 为准；历史条目中的“通过”只表示当时记录的验证结果。

本文记录已经进入代码库并通过验证的前端实现增量。它补充规范文档，不替代 `features/*` 中的契约。

## 2026-09-13 R1 社区 MVP 入口与匿名读取

- 作品详情社区区块拆分“写评价”和“参与讨论”入口；前者进入 Anime 评分/短评编辑页，后者进入讨论页，避免评分动作误进入评论页。
- 公开讨论允许匿名读取；发布、回复、编辑、删除和举报在讨论页显示登录门禁，登录后保留当前作品上下文。
- R1 编辑页只展示短评和讨论两种内容类型；长评入口后置，不影响后端已有长评协议。
- iOS 与 Android 复用同一套社区 Repository 和页面，Desktop/Web 仅作为预览测试宿主。
...
- 作品详情同时展示 Bangumi 只读评分和 Anime 社区评分；R1 MVP 开放评分、短评与讨论，社区内容失败不会覆盖资料状态，并提供全局轻提示。
