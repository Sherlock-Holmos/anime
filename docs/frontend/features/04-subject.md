...
路由 `AppRoute.Subject(subjectId: Long)`；ID `<= 0` 直接进入 Not Found，不发请求。详情包含 Hero、标题与元信息、Anime 社区评分、Bangumi 只读评分、简介、观看进度、剧集、角色、制作人员、关联条目、短评和讨论入口。
...
评分卡分别显示 Anime 社区评分与 Bangumi 来源评分，各自包含 `分数 / 10`、评分人数和来源标签；不得混算。Anime 评分入口允许登录用户进入 1–10 分编辑页；无对应评分时显示明确的空状态。
