...
受保护动作包括：修改收藏、修改进度、提交 Anime 评分、发短评/讨论、编辑或删除自己的内容、举报内容。公开作品资料、Anime/Bangumi 评分和公开社区内容允许匿名读取。匿名触发受保护动作时：保存一个 `PendingAuthAction` → 展示登录 Sheet → 登录成功后重新校验目标存在和参数合法 → 只重放一次。取消登录即丢弃动作。
