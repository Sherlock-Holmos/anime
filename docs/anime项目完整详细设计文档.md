...
> 维护说明：本文是 2026-07-21 的总体设计快照。当前产品范围、跨仓库事实源、发布判断和与现状的差异统一以 [`product/01-product-management-baseline.md`](product/01-product-management-baseline.md) 为准；本文中的历史“当前实现”表格待下一版重编。

> 文档版本：V1.4（CMP 与后端可执行契约基线）<br>
> 编制日期：2026-07-21<br>
> 项目代号：Anime<br>
> 文档用途：作为个人开发者与 AI Agent 协作时的产品、架构、接口、数据和验收基线<br>
> 当前交付平台：iOS IPA 侧载首发，Android 第二阶段，Desktop/Web 仅预览测试<br>
> iOS 状态：首发只提供可自行侧载的 IPA，不上架商店；宿主与平台适配源码已实现
...
| B-02 | 移动端优先；Desktop/Web 只做预览测试，不承诺生产发布 | 已确认 |
| B-03 | iOS IPA 侧载首发，Android 第二阶段；iOS 暂不上架商店 | 已确认 |
...
| B-20 | 先交付由 Fixture Repository 驱动、无需真实后端即可把玩的 CMP 移动端前端，再并行接入后端；R1 以 iOS IPA 首发、Android 第二阶段为生产顺序 | 已确认 |
...
| D-07 | Bangumi OAuth 用于绑定和双向同步；Anime 本地账号承担社区身份，二者最终都换成 Anime 会话 | 兼顾社区低门槛与 Bangumi 同步授权 |
| D-08 | Kyant Backdrop 只负责视觉效果，不由后端返回所谓 `glass_level` | 视觉策略属于客户端能力、页面状态和可访问性设置，不属于动漫领域数据 |
| D-09 | iOS 最低版本为 iOS 26；iOS 使用 SwiftUI 官方 Liquid Glass 宿主能力 | 与当前 Xcode 工程部署目标一致，支持 R1 IPA 侧载首发；App Store 发布另行决策 |
| D-10 | 客户端采用共享 UI + 共享业务逻辑，Android/iOS 宿主保持极薄，平台能力通过接口和 `expect/actual` 隔离 | 最大化 CMP 价值，同时保留平台差异能力 |
| D-11 | R1 MVP 建设 Anime 独立个人评分、社区聚合、短评和讨论；Bangumi 评分保持只读且严格分离 | 评分与评论是 Anime 的核心差异化，先做最小闭环，不建设复杂榜单 |
...
- Anime 本地账号登录，Bangumi OAuth 可选绑定并作为双向同步授权；
- 想看、在看、看过、搁置、抛弃状态，并与 Bangumi 双向同步；
- 观看进度记录及 Bangumi 双向同步；
- 展示 Bangumi 来源评分、投票数和评分分布，并明确标注来源；
- Anime 个人评分、社区聚合、短评、讨论和基础治理；
- 基础敏感词与频率控制；
- 本地缓存、离线回显和上游故障降级；
- iOS IPA 与 Android 液态玻璃主题以及低版本降级样式；
- 后端部署、日志、健康检查、备份与基础监控。

### 2.2 明确不在 MVP 范围内

- Web/桌面生产发布；Desktop/Web 仅作为预览、联调和回归测试宿主；
- 动漫视频播放、下载、转码或播放源聚合；
- 私信、群组和实时聊天；
- 专栏系统和复杂富文本编辑器；
- OCR 截图识番；
...
- iOS App Store 上架；iOS IPA 侧载和真机验收属于当前 MVP 范围。
...
- Bangumi OAuth 是双向同步凭据来源；R1 社区身份可来自 Anime 本地账号；
- 登录时请求最小必要 scope；
