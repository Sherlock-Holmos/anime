...
| FR-042 | 不提供 Anime 自有评分与榜单 | P0 | 客户端没有本地评分入口，API 不暴露本地评分写接口或 Anime 评分榜单 |
...
MVP 不计算 Anime 自有评分。评分相关字段均视为 Bangumi 来源快照：
...
- 未来如建设 Anime 自有评分体系，必须通过新的产品决策、ADR、API 版本和数据库迁移单独引入。
...
| Q-07 | MVP 评分策略 | 只展示 Bangumi API 的评分、投票数和分布，不建设 Anime 自有评分、加权榜单或评分写入 |
| Q-08 | 推送提醒 | MVP 不实现 |
| Q-09 | 项目正式名称 | Anime |
